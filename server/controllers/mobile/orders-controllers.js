...
const getOrders = async (req, res, next) => {
  ...
  try {
    orders = await Orders.find({ userId: userId })
      .select("totalItems currentStatus orderImage timestamp")
      .sort({ timestamp: "desc" });
  } catch (error) {
  ...
  if (!orders) {
    return next(
      new HttpError("Could not find a order for the provided user.", 404)
    );
  }
...
const getOrderById = async (req, res, next) => {
  ...
  try {
    ...
    return next(
      "Something went wrong, could not able to find order for given id.",
      500
    );
  }
