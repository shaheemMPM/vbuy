...
const getCategoriesByShopId = async (req, res, next) => {
  ...
  try {
    ...
    return next(
      new HttpError("Reading categories with given shop id failed.", 500)
    );
  }
  ...
  if (!categories) {
    return next(
      new HttpError(
        "Could not find a categories for the provided shop id.",
        404
      )
    );
  }
...
const deleteCategory = async (req, res, next) => {
  ...
  try {
    ...
    return next(
      new HttpError("Could not find a category for the provided id.", 500)
    );
  }
  ...
  try {
    ...
    return next(
      new HttpError(
        "Could not find subcategories for the provided categoryId",
        500
      )
    );
  }
  ...
  if (subcategories) {
    try {
      ...
      return next(
        new HttpError(
          "Could not delete subcategories for the provided categoryId",
          500
        )
      );
    }
  ...
  try {
    ...
    return next(
      new HttpError("Could not find products for the provided categoryId", 500)
    );
  }
  ...
  if (products) {
    try {
      ...
      return next(
        new HttpError(
          "Coult not delete products for the provided categoryId",
          500
        )
      );
    }
  ...
  try {
    ...
    return next(
      new HttpError("Could not delete a category for the provided id.", 500)
    );
  }
